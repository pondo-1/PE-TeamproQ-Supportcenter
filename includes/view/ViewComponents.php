...
{
  $icon = '<div class="icon-and-capture"><span class="icon-icon-' . get_post_field('post_name', $postId) . '"></span>
</div>';
...
function supportcenter_header()
{
  ...
  elseif (is_singular('supportcenter') && !get_post_parent($postId)) {
    // Add modul description to header
    $icon = '<div class="icon-and-capture"><span class="icon-icon-' . get_post_field('post_name', $postId) . '"></span></div>';
    $content = '<div class="modul_content">' . get_the_content($postId) . '</div>';
